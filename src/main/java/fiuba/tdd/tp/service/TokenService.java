...
public class TokenService {
    ...
    // Recibe un usuario autenticado y genera un token que expira en una hora
    public String generarToken(Authentication autenticacion) {
        Instant tiempoActual = Instant.now();
        String scope = autenticacion.getAuthorities().stream()
                            .map(GrantedAuthority::getAuthority)
                            .collect(Collectors.joining(" "));
        JwtClaimsSet claims = JwtClaimsSet.builder()
                            .issuer("self")
                            .issuedAt(tiempoActual)
                            .expiresAt(tiempoActual.plus(1, ChronoUnit.HOURS))
